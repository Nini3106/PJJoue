...
def entete(page,captures=False):
 ...
 atlasVisible:/\\batlas\\b/i.test(document.body.innerText)};
    ...
    assert result['texte']==result['logoFond']==result['cjpmContour']=='rgb(244, 240, 232)',result
    ...
    assert all(l['couleur']=='rgb(244, 240, 232)' for l in result['liens'])
    ...
    if page.locator('#boutonMenuMobile').count():
        ...
        assert page.locator('#boutonParcoursPJJ').evaluate('e=>getComputedStyle(e).color')=='rgb(244, 240, 232)'
    else:
