...
`08 - Réviser/Jeu des sigles` contient le mini-jeu **Mission Sigles**. Ses données ne sont pas dupliquées dans la page : la liste canonique des 96 sigles (51 CJPM et 45 PJJ) se trouve dans `donnees/sigles.json`. Le même fichier alimente le support de révision, les guides `sigles-cjpm/` et `sigles-pjj/` et les activités du mini-jeu lors de la construction.
