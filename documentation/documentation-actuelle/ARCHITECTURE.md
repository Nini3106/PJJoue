# Architecture technique de PJJoue

> **V1 — point d’entrée du code :** ouvre d’abord le dossier `code/`. Il est rangé par nom de page visible. Les fichiers publics `index.html`, `ressources/moteur-jeu.js` et les feuilles CSS sont reconstruits par `python outils/construire_site.py`. Pour la carte simple, voir `documentation/documentation-actuelle/ARCHITECTURE_CODE_PAR_PAGE.md`.
...
Les répertoires `decouvrir-la-pjj/`, `organisation-pjj/`, `metiers-pjj/`, `structures-pjj/`,
`mesures-educatives-pjj/`, `sigles-pjj/` et `quiz-pjj/`
...
Le bouton principal de l’accueil est actualisé par `actualiserBoutonCommencer`. Il propose de commencer lorsque la progression est vide, puis de reprendre la première étape utile ou d’ouvrir le carnet lorsque le voyage est complet.
...
> Confidentialité Analytics : le texte des questions n’est pas transmis à Google Analytics ; seul l’identifiant permanent de la question est utilisé pour le suivi.
