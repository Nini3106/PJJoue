...
Les identifiants techniques stables servent à conserver l'historique lorsqu'un contenu évolue. Les questions sont suivies uniquement par leur identifiant permanent `pjjoue_identifiant_question` ; leur énoncé n’est pas transmis à Google Analytics.
...
Il est normal que certains paramètres soient absents d'un événement : par exemple `pjjoue_son` ne concerne que `pjjoue_parametres_enregistres`, tandis que `pjjoue_identifiant_question` concerne les événements de question. **Ne pas inventer de valeur de remplacement** pour un paramètre qui n'est pas pertinent.
...
`pjjoue_identifiant_question` contient uniquement l’identifiant permanent de la question, par exemple `Q001` ou `Q1708`. **L’identification exacte doit toujours se faire avec ce paramètre.** L’énoncé n’est pas transmis à Google Analytics.
